...
int nAnonymizephantomxAmount = 500;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\phantomx
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\phantomx
    // Mac: ~/Library/Application Support/phantomx
    // Unix: ~/.phantomx
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "phantomx";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "phantomx.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "phantomxd.pid"));
