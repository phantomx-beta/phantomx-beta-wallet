...
memory available when compiling PhantomX Core. With 1024MB of memory or less
...
build libsecp256k1
```
cd ~/phantomx-beta-wallet/src/secp256k1
./autogen.sh
...
go back to Phantomxcoin dir to modify phantomx.pro if needed :
```
cd ~/phantomx-beta-wallet
nano phantomx-beta-wallet.pro
```
...
### Compiling phantomx-beta-walletd
With dependencies (except QT cause not needed here) compliled as above, put these lines in ~/phantomx-beta-wallet/src/makefile.unix (first lines)
```
DEPS_PATH = $(HOME)/deps
SECP256K1_LIB_PATH = src/secp256k1/.libs
SECP256K1_INCLUDE_PATH = src/secp256k1/include
...
OPENSSL_INCLUDE_PATH = $(DEPS_PATH)/openssl-1.0.2g/include
```
After that in ~/phantomx-beta-wallet/src
```
make -f makefile.unix
strip phantomxd
```
